...
import { GOOGLE_ACCESS_TOKEN_URL, GOOGLE_OAUTH_REDIRECT_URI, GOOGLE_OAUTH_ROOT_URL } from "../../constants";
...
export const handleGoogleOAuthCallback = async (req: Request, res: Response) => {
    ...
    try {
        const response = await axios.post(GOOGLE_ACCESS_TOKEN_URL, qs.stringify({
            code,
            client_id: process.env.GOOGLE_CLIENT_ID!,
        ...
        const accessToken = generateAccessToken(user.id, user.email, user.username, `${user.firstname} ${user.lastname}`)
        ...
        res.status(500).json({ message: 'Failed to exchange code for tokens', error });
    }
}